...
const assignedHues = new Map();  // session id -> robot colour, when it differs from the usual
...
// What a tool call is about, short: the file name for file tools, else the command
function toolTarget(toolName, toolInput, max) {
  let target = summarizeTool(toolName, toolInput);
  if (FILE_TOOLS.has(toolName) && target) target = path.basename(target);
  return oneLine(target, max);
}

function doing(toolName, toolInput) {
  if (!toolName) return 'Thinking…';
  const target = toolTarget(toolName, toolInput, 70);
...
function deriveState(s, info, ctx, now) {
  ...
  if (perms.length) {
    const p = perms[0];
    const target = toolTarget(p.toolName, p.toolInput, 60);
    return {
...
function build(ctx) {
  ...
  // A brand-new Room robot shows up in Claude's session list before Claude
  // tells us its id; match it by process id so it doesn't flicker
  for (const run of runner.runs.values()) {
    if (run.status !== 'running' || run.sessionId) continue;
    for (const [id, entry] of registry) if (entry.pid === run.pid) run.sessionId = id;
  }
  // ...and it keeps the name and colour it had while it was starting
  for (const run of runner.runs.values()) {
    const early = assignedNames.get(`run-${run.runId}`);
    if (run.sessionId && early && !assignedNames.has(run.sessionId)) {
      assignedNames.set(run.sessionId, early);
      assignedHues.set(run.sessionId, hash(`run-${run.runId}`) % 360);
    }
  }
  ...
  for (const s of byId.values()) {
    ...
    // You removed it (or its room) from the Room: stay away until it does something new
    const removedAt = cleanup.hiddenAt(s.id, roomKey(cwd));
    if (removedAt && lastTouch <= removedAt && !s.hasPermission && !runActive) continue;
    ...
    const session = {
      ...
      hue: assignedHues.has(s.id) ? assignedHues.get(s.id) : h % 360,
      state: st.state,
    ...
    // You can reply from the Room whenever the robot isn't busy. If the chat is
    // also open in the Claude app or a terminal, the Room still continues the
    // same conversation — that window just won't show it until it's reopened.
    const busy = st.state === 'working' || (st.state === 'blocked' && st.reason !== 'error');
    session.busy = busy;
    session.openElsewhere = !!s.reg && !runActive;
    session.canReply = runner.cli.found && !session.placeholder && !runActive && !busy;
    session.canStop = st.state === 'working' || st.state === 'blocked';

    // Quick scan: helpers, connections, files changed
    let q = { helpersActive: 0, helpersTotal: 0, connections: [], filesChanged: 0 };
    if (f) {
      try { q = scan.quick(f, st.state === 'working'); } catch { /* unreadable right now */ }
    }
    session.helpers = { active: q.helpersActive, total: q.helpersTotal };
    session.connections = q.connections;
    session.filesChanged = q.filesChanged;
