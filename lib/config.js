...
const DEFAULTS = {
  ...
  notify: false,              // desktop notifications (the browser asks first)
  scale: 3,                   // pixel zoom
  roomStyle: 'mixed',         // mixed (every room different), office, neon, space or cabin
};
const ROOM_STYLES = ['mixed', 'office', 'neon', 'space', 'cabin'];
...
function saveSettings(changes) {
  for (const key of Object.keys(DEFAULTS)) {
    ...
    if (want === 'string') value = String(value);
    if (want === 'number' && !Number.isFinite(value)) continue;
    settings[key] = value;
  }
  if (!ROOM_STYLES.includes(settings.roomStyle)) settings.roomStyle = 'mixed';
